...
from .models import SessionYearModel, AcademicTerm


@receiver(post_save, sender=SessionYearModel)
def after_saving_session(sender, created, instance, *args, **kwargs):
    ...
    if instance.current is True:
        SessionYearModel.objects.exclude(pk=instance.id).update(current=False)
